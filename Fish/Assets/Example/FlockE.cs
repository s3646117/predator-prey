...
public class FlockE : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (transform.parent)
        {
            tempFlocks = transform.parent.GetComponentsInChildren<FlockE>();
        }
        
        //Assign and store all the flock objects in this group
        objects = new Transform[tempFlocks.Length];
        otherFlocks = new FlockE[tempFlocks.Length];
        for (int i = 0; i < tempFlocks.Length; i++)
        {
            objects[i] = tempFlocks[i].transform;
            otherFlocks[i] = (FlockE)tempFlocks[i];
        }
        ...
        for (int i = 0; i < objects.Length; i++)
        {
            ...
            if (transform != transformComponent)
            {
                ...
                if (d < followRadius)
                {
                    ...
                    f = d / followRadius;
                    FlockE otherSealgull = otherFlocks[i];
